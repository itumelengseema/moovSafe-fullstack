...
export default function InspectionFormScreen() {
  ...
  const [faultImages, setFaultImages] = useState<ImagePicker.ImagePickerAsset[]>([]);
  const [odometerImage, setOdometerImage] = useState<ImagePicker.ImagePickerAsset | null>(null);
  ...
  const takeFaultPhoto = async () => {
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        Alert.alert('Photo Captured', 'Fault photo has been added successfully!', [{ text: 'OK' }]);
      }
    } catch (_error) {
  ...
  const takeOdometerPhoto = async () => {
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        // Show success message
        Alert.alert('Photo Captured', 'Odometer photo has been captured successfully!', [
          { text: 'OK' },
        ]);
      }
    } catch (_error) {
  ...
  const submitInspection = async () => {
    ...
    try {
      ...
      formData.append('mileage', parseInt(mileage, 10).toString());
  ...
  if (!currentInspection) {
    return (
      <ScrollView className="flex-1 p-4">
        <Card className="p-6">
          <Text>No inspection in progress. Please start an inspection from a vehicle page.</Text>
      ...
      <VStack space="lg">
            ...
            <VStack space="sm">
              ...
              <HStack space="sm" className="flex-wrap">
                {CONDITION_OPTIONS.map((option) => (
                  <TouchableOpacity
                    key={option}
                    onPress={() => handleFieldChange('overallCondition', option)}
                  ...
                  >
                    <Text
                      className={
                        currentInspection.overallCondition === option ? 'text-white' : 'text-black'
                      }
                    >
                      {option}
                    </Text>
                  ...
                  {field.options.map((option) => (
                    <TouchableOpacity
                      ...
                      className={`px-3 py-2 rounded-lg border ${
                        currentInspection[field.key as keyof typeof currentInspection] === option
                          ? 'bg-blue-500 border-blue-500'
                          : 'bg-white border-gray-300'
                      }`}
                    >
        ...
        <Card className="p-4">
          <VStack space="md">
            ...
            <VStack space="sm">
              ...
              {odometerImage ? (
                <VStack space="sm">
                  ...
                  />
                  <HStack space="sm">
                    <Button onPress={takeOdometerPhoto} size="sm" className="flex-1 bg-blue-600">
            ...
            {/* Fault Images */}
            <VStack space="sm">
              <Text className="font-semibold">Fault Images (Optional - Max 5)</Text>
